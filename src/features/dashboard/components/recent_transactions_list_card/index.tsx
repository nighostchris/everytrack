...
export const RecentTransactionsListCard: React.FC<RecentTransactionsListCardProps> = ({ data, className }) => {
  ...
  return (
    <Card className={clsx('flex flex-col !bg-white p-6 md:h-96', className)}>
      ...
      <div className="grid h-full w-full">
        {data.length > 0 ? (
          data.map(({ name, category, amount, income }, index) => (
            <div className={clsx('grid grid-cols-5 py-2 md:py-0', { 'border-t border-gray-100': index !== 0 })}>
              <div className="col-span-3 flex flex-col justify-center md:hidden">
              ...
              <div className="col-span-2 hidden flex-row items-center text-sm text-gray-700 md:flex">{name}</div>
              <div className="col-span-2 hidden flex-row items-center md:flex">
              ...
              <div
                className={clsx('col-span-2 flex flex-row items-center justify-end text-sm md:col-span-1', {
